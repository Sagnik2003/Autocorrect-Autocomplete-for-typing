...
MODEL_FILE = r"Autocorrect-Autocomplete-for-typing/App/data/language_model_data.pkl" 
...
def load_model(filename=MODEL_FILE):
    ...
    try:
        ...
        return data['vocab'], data['n_gram_counts_list']
    except FileNotFoundError:
        print(f"Error: Model file {filename} not found. Must train the model first.")
        return None, None
    except KeyError as e:
        print(f"Error loading model: Key {e} not found in the model file. Re-run training (TRAINING_MODE=True).")
        return None, None
...
def get_suggestions(previous_tokens, n_gram_counts_list, vocabulary, k=1.0, start_with=None, n_suggestions=5):
    ...
    # We loop up to model_counts - 1 because we need N-gram (context) and (N+1)-gram (prediction)
    for i in range(model_counts - 1): 
        n_gram_counts = n_gram_counts_list[i] 
        n_plus1_gram_counts = n_gram_counts_list[i+1] 
        
        # Get suggestions from this model (we ask for more suggestions here to ensure we get enough unique ones)
        # The inner function handles its own sorting and limiting, but we still need the overall top N.
        model_suggestions = suggest_a_word(
            previous_tokens, n_gram_counts, n_plus1_gram_counts, 
            vocabulary, k=k, start_with=start_with, n_suggestions=n_suggestions * 2 # Request double the amount
        )
...
if __name__ =="__main__":
    ...
    # --- PREDICTION / DEPLOYMENT MODE (Simplified) ---
        ...
        print(f"\nEvaluating sequence and predicting next word after context: {context_words}")
